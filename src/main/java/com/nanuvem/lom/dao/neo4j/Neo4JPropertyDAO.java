...
public class Neo4JPropertyDAO implements PropertyDao{
	...
	public Neo4JPropertyDAO(Neo4JConnector connector, Neo4JEntityDao entityDao, Neo4JPropertyTypeDao propertyTypeDao) {
	...
	public Property create(Property property) {
		Node entityNode = this.entityDao.findNodeById(property.getEntity().getId());
		Node propertyTypeNode = this.propertyTypeDao.findNodeById(property.getPropertyType().getId());
	...
	public Property update(Property property) {
		String query = "MATCH (p:" + NodeType.PROPERTY + " {"+ "id: " + property.getId() + "}) " 
				+ " SET" 
				+ " p.version= '" + property.getVersion()
				+ " return p";
		try (Transaction tx = connector.iniciarTransacao();
	...
	public Property findById(Long id) {
		String query = "MATCH "
				+ "(p:" + NodeType.PROPERTY + ")-[er]->(e:"+NodeType.ENTITY+"), "
				+ "(p:" + NodeType.PROPERTY + ")-[pr]->(pt:"+NodeType.PROPERTY_TYPE + ")"
		...
		try (Transaction tx = connector.iniciarTransacao();
			...
			if (result.hasNext()) {
				...
				PropertyType propertyType = this.propertyTypeDao.newPropertyType(nodePT);
	...
	private Property newProperty(Node node) {
